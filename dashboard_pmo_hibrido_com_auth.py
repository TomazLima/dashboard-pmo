...
def configurar_analise_hibrida():
    """Configuração para análise online (Llama) + offline avançada"""
    st.sidebar.markdown("---")
    ...
    api_key = st.sidebar.text_input(
        "API Key do Groq (Opcional):",
        value=api_key_default,
        type="password",
        help="Chave GRATUITA do Groq para análise online"
    )
    
    # Toggle para modo online
    usar_online = st.sidebar.toggle(
        "🦙 Análise Online (Llama 3.3)",
        value=bool(api_key),
        help="Ativar análise com Llama 3.3 via Groq"
    )
    ...
    return api_key if usar_online else None
...
def criar_sidebar(df, metricas):
    ...
    st.sidebar.markdown("*Modelo para Insights: Online + Offline*")
    
    # Configuração da análise híbrida
    api_key = configurar_analise_hibrida()
...
def analise_llama_online(df, metricas, api_key):
    """Análise online com Llama 3.3 incluindo novos indicadores"""
    ...
    try:
        ...
        # Novos dados de cronograma
        cronograma = analisador.calcular_indicadores_cronograma(df)
        previsao = analisador.prever_conclusao_projeto(df)
        
        # Construir prompt avançado
        prompt = f"""Analise este projeto PMO Deutsche Telco com dados avançados de cronograma:

📊 **DADOS GERAIS:**
• Total: {total} atividades
• Conclusão geral: {conclusao}%
• Status: {dict(metricas['status_counts'])}

⏰ **INDICADORES DE CRONOGRAMA:**
• Aderência aos prazos: {cronograma['aderencia_prazo']}%
• Atividades atrasadas: {cronograma['atrasadas']}
• Vencendo em 7 dias: {cronograma['vencendo_semana']}
• Atraso médio: {cronograma['dias_atraso_medio']} dias
• SLA validação: {cronograma['sla_validacao']} dias

🔮 **PREVISÃO:**
{f"Data prevista conclusão: {previsao['data_prevista'].strftime('%d/%m/%Y')}" if previsao else "Sem previsão disponível"}
{f"Possível atraso: {previsao['dias_adicao']} dias" if previsao and previsao['dias_adicao'] > 0 else ""}

Como especialista PMO, identifique os 2 maiores RISCOS TEMPORAIS e 2 AÇÕES mais urgentes.

Formato:
**🚨 RISCOS TEMPORAIS:**
1. [risco crítico relacionado a prazos]
2. [segundo risco temporal]

**⚡ AÇÕES URGENTES:**
1. [ação mais crítica para cronograma]
2. [segunda ação prioritária]"""
        ...
        if response.status_code == 200:
            ...
            analise_completa = f"**🦙 ANÁLISE LLAMA 3.3 ONLINE - CRONOGRAMA AVANÇADO:**\n\n{analise}"
            
            return {
                "analise": analise_completa,
                "custo": 0.000,
                "modo": "online-cronograma",
                "ok": True
            }
        else:
            return {"erro": f"API erro {response.status_code}", "ok": False}
...
def widget_analise_hibrida(df, metricas, api_key):
    """Widget para análise híbrida (online + offline avançado)"""
    
    st.markdown("---")
    st.subheader("🤖 AI Insights (Llama 3.3) + Versão Offline")
    ...
    with col1:
        ...
        with col_online:
            if api_key:
                if st.button("🤖 AI Insights", type="primary"):
                    with st.spinner("🤖 Llama 3.3 analisando cronograma..."):
                        resultado = analise_llama_online(df, metricas, api_key)
                    st.session_state['analise_resultado'] = resultado
            else:
                st.button("🤖 AI Insights", disabled=True, help="API Key do Groq necessária")
    ...
    if 'analise_resultado' in st.session_state:
        ...
        if resultado.get("ok"):
            ...
            # Para análise online (Llama)
            if resultado.get("modo") == "online-cronograma":
                st.success("✅ **Análise Online Concluída:**")
                st.markdown(resultado["analise"])
                st.caption("🤖 Powered by Llama 3.3-70b com foco em cronograma | GRATUITO!")
...
def main():
    ...
    st.markdown("**Modelo para Insights: Online (Llama 3.3) + Offline**")
    ...
    widget_analise_hibrida(df_filtrado, metricas_filtradas, api_key)
    ...
    with st.expander("🎯 Principais Insights", expanded=True):
        ...
        with col1:
            st.markdown("""
            **✅ Novidades v5:**
            - 🦙 Análise híbrida (Online + Offline)
    ...
    with st.expander("🚀 Como Usar o Dashboard v5"):
        ...
        **🦙 Análise Online (Llama 3.3):**
        ...
        2. Cole sua API Key na sidebar
        3. Ative "Análise Online"
        4. Clique em "Análise Online" para insights com IA
        ...
        **⏰ Novos Indicadores de Cronograma:**
        - **Aderência aos Prazos:** % atividades no prazo
        - **Timeline Visual:** Roadmap com datas
        - **Situação dos Prazos:** Atrasadas vs Futuras
        - **SLA de Validação:** Tempo médio de aprovação
        - **Previsão de Conclusão:** Data estimada final
