...
def load_sample_data():
    ...
    sample_data = [
        {"id": "1.1", "fase": "Initial Milestone", "atividade": "Kick-Off", "status": "Completed", "responsavel": "OS", "data_prevista": "2025-02-12", "progresso": 100, "peso": 1.0, "observacoes": "Done", "delivery": "Sprint 1", "Dimensões": "Process"},
        {"id": "2.1", "fase": "Phase 1", "atividade": "Diagnosis", "status": "Awaiting Validation", "responsavel": "DT", "data_prevista": "2025-04-07", "progresso": 80, "peso": 0.8, "observacoes": "Awaiting validation", "delivery": "Sprint 2", "Dimensões": "Technology"},
        {"id": "2.2", "fase": "Phase 1", "atividade": "Gap Analysis", "status": "In Progress", "responsavel": "OS", "data_prevista": "2025-05-30", "progresso": 50, "peso": 0.5, "observacoes": "In progress", "delivery": "Sprint 3", "Dimensões": "People"},
        {"id": "3.1", "fase": "Phase 2", "atividade": "Structuring", "status": "Identified", "responsavel": "To Be Defined", "data_prevista": "2025-06-30", "progresso": 0, "peso": 0.0, "observacoes": "Pending", "delivery": "Sprint 4", "Dimensões": "Governance"},
    ]
...
def calculate_metrics(df):
    ...
    # Debug: Print status counts to understand the data
    print("Debug - Status counts:", status_counts.to_dict())
    print("Debug - Unique status values:", df['status'].unique().tolist())
...
def calculate_schedule_indicators(df):
    ...
    overdue = len(df_with_dates[
        ...
        (df_with_dates['status'] != 'Completed')
    ])
    ...
    completed = len(df_with_dates[df_with_dates['status'] == 'Completed'])
...
def create_status_donut(df):
    ...
    # More flexible color mapping to handle different languages
    def get_status_color(status):
        status_lower = status.lower()
        
        # Completed/Concluído - Magenta
        if any(word in status_lower for word in ['completed', 'concluído', 'concluido', 'done', 'finished']):
            return '#E20074'
        
        # In Progress/Em Andamento - Light Magenta  
        elif any(word in status_lower for word in ['progress', 'andamento', 'working', 'desenvolvimento']):
            return '#F066A7'
        
        # Awaiting/Aguardando - Pastel Magenta
        elif any(word in status_lower for word in ['awaiting', 'aguardando', 'validação', 'validation', 'pending']):
            return '#F899C9'
        
        # Identified/Identificado - Gray
        elif any(word in status_lower for word in ['identified', 'identificado', 'planned', 'planejado']):
            return '#8E8E93'
    ...
    print("Debug - Status and colors:")
    for i, status in enumerate(status_counts.index):
        print(f"  {status}: {colors[i]}")
    
    fig = go.Figure(data=[go.Pie(
        labels=status_counts.index,
        values=status_counts.values,
        hole=0.6,
        marker=dict(colors=colors, line=dict(color='#FFFFFF', width=2)),  # Added explicit marker dict
        textinfo='label+percent',
        textfont_size=10,
        textposition="outside",
        hovertemplate='<b>%{label}</b><br>Quantity: %{value}<br>Percentage: %{percent}<extra></extra>',
        pull=[0.05 if any(word in status.lower() for word in ['completed', 'concluído', 'concluido']) else 0.02 for status in status_counts.index]
    )])
...
def main():
    ...
    with col3:
        # Find completed activities more flexibly
        completed = 0
        for status in filtered_metrics['status_counts'].index:
            # Check for different variations of "completed"
            status_lower = status.lower()
            if any(word in status_lower for word in ['completed', 'concluído', 'concluido', 'done', 'finished']):
                completed += filtered_metrics['status_counts'][status]
