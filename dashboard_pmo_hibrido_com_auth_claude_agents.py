...
# ============================================
# 🤖 IMPORTAR MULTI-AGENTES
# ============================================
try:
    import multi_agents_pmo
...
def criar_sidebar(df, metricas):
    ...
    # 🆕 SEÇÃO DE AGENTES NA SIDEBAR
    if AGENTS_AVAILABLE and 'multi_agents' in st.session_state:
        multi_agents_pmo.render_agents_sidebar(st.session_state.multi_agents)
...
def main():
    ...
    st.markdown("**Modelo Premium: Online (Claude) + Offline Avançado + Multi-Agentes 🤖**")
    ...
    if AGENTS_AVAILABLE:
        multi_agents = multi_agents_pmo.integrate_agents_to_dashboard(df_filtrado, metricas, api_key)
    else:
        multi_agents = None
    ...
    # ============================================
    # 🤖 MULTI-AGENTES WIDGET
    # ============================================
    
    if AGENTS_AVAILABLE and multi_agents:
        st.markdown("---")
        st.subheader("🤖 AI Multi-Agents Team")
        
        # Widget compacto de status
        multi_agents_pmo.render_agents_status_widget(multi_agents)
        ...
        with col1:
            if st.button("🚀 Execute Multi-Agent Analysis", type="primary"):
                if api_key:
                    with st.spinner("🤖 AI Agents analyzing your project..."):
                        result = multi_agents.execute_analysis(df_filtrado, metricas_filtradas)
                    
                    if result["success"]:
                        st.success(f"✅ Multi-Agent analysis completed in {result['execution_time']:.1f}s")
                        
                        with st.expander("🤖 Multi-Agent Analysis Results", expanded=True):
                            st.markdown("### 📊 Collaborative AI Insights")
                            ...
                            with col_b:
                                st.metric("Tasks Completed", result["tasks_completed"])
                            with col_c:
                                st.metric("Execution Time", f"{result['execution_time']:.1f}s")
                    else:
                        st.error(f"❌ Multi-Agent Error: {result['error']}")
                else:
                    st.warning("⚠️ Anthropic API Key required for multi-agent analysis")
